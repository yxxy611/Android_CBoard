...
public class PaletteView extends View {
    ...
    private void init() {
        ...
        Bitmap tempBitmap = Bitmap.createBitmap(mBitmaps[countNow]);
        initCache();
    ...
    public void Test1() {
       ...
       //testPath1.setLastPoint(500,500);
        ...
        invalidate();*/
        //cacheBitmapsList.get(0) = Bitmap.createBitmap(mBitmaps[countNow]);
    }
    ...
    /*private void saveCacheBitmaps(){
        int index = cacheBitmapsList.size();

        Bitmap tempBitmap = Bitmap.createBitmap(mBitmaps[countNow]);
        //Log.e("Lilith","Width =" + tempBitmap.getWidth());
        //Log.e("Lilith","Height =" + tempBitmap.getHeight());
        if(index > 10){
            cacheBitmapsList.remove(0);
            cacheBitmapsList.add(tempBitmap);
        }else{
            cacheBitmapsList.add(tempBitmap);
            cacheIndex++;
    ...
    ArrayList<UnDo> mUnDoList = new ArrayList<>();

    public void saveUnDoInfo(int i){
        if(mIsLassoMove){
            Path path = new Path();
        }
    }
    ...
    public void unDo(){
        Log.e("Lilith","index =" + cacheIndex);
        if(cacheIndex > 0){
            cacheIndex--;
            mBitmaps[countNow] = Bitmap.createBitmap(cacheBitmapsList.get(cacheIndex));
            //Log.e("Lilith","Width =" + cacheBitmapsList.get(cacheIndex).getWidth());
            //Log.e("Lilith","Height =" + cacheBitmapsList.get(cacheIndex).getHeight());
    ...
    public void reDo(){
        Log.e("Lilith","index =" + cacheIndex);
        if(cacheIndex < cacheBitmapsList.size() - 1){
            cacheIndex++;
            mBitmaps[countNow] = Bitmap.createBitmap(cacheBitmapsList.get(cacheIndex));
            mBitmapCanvas.setBitmap(mBitmaps[countNow]);
            invalidate();
    ...
    private void BuildLassoArea() {
        ...
        for (int i = 0; i < mAreaPathsList.size(); i++) {
            if (mPathStack.get(i).mPageIndex == countNow) {
  ...
  /*      this.mShaderMatrix.set(null);
    ...
    private void draw(Canvas canvas, CopyOnWriteArrayList<GraffitiPath> pathStack, int index) {
        ...
        for (int i = 0; i <= index; i++) {
            if (pathStack.get(i).mPageIndex == countNow) {
    ...
    public void setBitmapScale(float scale) {
        ...
        Bitmap resizeBmp = Bitmap.createBitmap(mBitmaps[countNow],0,0,mBitmaps[countNow].getWidth(),mBitmaps[countNow].getHeight(),matrix,true);
...
/*        if (mBitmap.isRecycled() || mBitmaps[countNow].isRecycled()) {
    ...
    private void setBG() {// 不用resize preview
        int w = mBitmaps[countNow].getWidth();
        int h = mBitmaps[countNow].getHeight();
