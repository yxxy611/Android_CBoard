...
import static com.SiWei.PaintingApp.DrawUtil.restoreRotatePointInGraffiti;
import static com.SiWei.PaintingApp.DrawUtil.rotatePoint;
import static com.SiWei.PaintingApp.DrawUtil.rotatePointInGraffiti;
...
public abstract class GraffitiSelectableItem{

    public final static int ITEM_CAN_ROTATE_BOUND = 80;

    private PaletteView.Pen mPen;
    private float mSize;
    ...
    public GraffitiSelectableItem(PaletteView.Pen pen, float size, PenColor color, int textRotate, int rotateDegree, float x, float y, float px, float py) {
        this.mPen = pen;
        this.mSize = size;
        this.mColor = color;
     ...
     * 是否击中
     */
    public boolean isInIt(int currentRotate, float x, float y, PaletteView.Pen pen) {
        if (pen != mPen) {
            return false;
        }
    ...
    public abstract void draw(Canvas canvas, PaletteView penView, Paint paint);
}
